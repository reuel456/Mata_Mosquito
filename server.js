...
// Controle de Clique por Gestos nos Botões
let botaoAtual = null;
let tempoSobreBotao = null;
...
function inicializarMediaPipe() {
    ...
    if (ponteiro) {
        // Garante que o ponteiro não bloqueie o clique no elemento de baixo
        ponteiro.style.pointerEvents = 'none';
    }

    if (typeof Hands === 'undefined' || typeof Camera === 'undefined') {
        console.warn('MediaPipe ainda não carregado.');
        return;
    }
...
function onResultsHands(results) {
    if (results.multiHandLandmarks && results.multiHandLandmarks.length > 0) {
        ...
        const posX = (1 - indicador.x) * window.innerWidth;
        const posY = indicador.y * window.innerHeight;

        if (ponteiro) {
            ponteiro.style.left = `${posX}px`;
            ponteiro.style.top = `${posY}px`;
            ponteiro.style.display = 'block';
        }

        if (jogoAtivo) {
            checarColisaoComMosquito(posX, posY);
        } else {
            // Se estiver nos menus, permite clicar em botões segurando o dedo em cima
            checarInteracaoBotoes(posX, posY);
        }
    } else {
...
// Lógica de Clique por Permanência em Botões
function checarInteracaoBotoes(x, y) {
    if (cooldownClique) return;

    const elemento = document.elementFromPoint(x, y);
    const botao = elemento ? elemento.closest('button, input[type="submit"], .btn') : null;

    if (botao) {
        if (botaoAtual !== botao) {
            limparFocoBotao();
            botaoAtual = botao;
            botaoAtual.classList.add('foco-mao');

            // Clica no botão após 0.8s segurando o dedo sobre ele
            tempoSobreBotao = setTimeout(() => {
                botaoAtual.click();
                cooldownClique = true;
                limparFocoBotao();

                // Evita múltiplos cliques involuntários seguidos
                setTimeout(() => { cooldownClique = false; }, 1200);
            }, 800);
        }
    } else {
        limparFocoBotao();
...
function limparFocoBotao() {
    if (botaoAtual) {
        botaoAtual.classList.remove('foco-mao');
        botaoAtual = null;
    }
    if (tempoSobreBotao) {
        clearTimeout(tempoSobreBotao);
        tempoSobreBotao = null;
    }
}

function checarColisaoComMosquito(x, y) {
